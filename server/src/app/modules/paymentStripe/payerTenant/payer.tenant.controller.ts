...
class PayerTenantController {
    ...
    static createTenantPaymentIntent = catchAsync(async (req: Request, res: Response) => {
        ...
        sendResponse(res, {
            ...
            message:
                httpStatusCode === 201
                    ? this.orderCreationSuccessMessage
                    : this.orderCreationFailedMessage,
            data: jsonResponse,
        });
    });
    ...
    static retriveTenantPaymentInformation = catchAsync(async (req: Request, res: Response) => {
        ...
        const { jsonResponse, httpStatusCode } = await TenantPaymentProcessor.retrivePaymentInfo(paymentIntentId, connectedAccountId)
